...
class Graph_3D_edges:
    ...
    def get_neighbors_lists(self, connections,num_neigh=None):
        ...
        """
        if num_neigh == None:
            self.neighbors_mat = Graph_3D_edges.get_neighbors(self)
        if num_neigh != None:
            self.neighbors_mat = Graph_3D_edges.get_neighbors_reduced(self,num_neigh)
        self.neighbors_lists = [Graph_3D_edges.get_neighbors_list(self, connections[i], self.neighbors_mat[i]) \
                                for i in range(0,len(self.vert))]
    ...
    def get_connections(self, edges,num_neigh=None):
        ...
        if num_neigh != None:
            self.neighbors_mat = Graph_3D_edges.get_neighbors_reduced(self,num_neigh)
        self.connections = [Graph_3D_edges.get_connection(self, self.vert[i], edges, self.neighbors_mat[i]) \
                            for i in range(0,len(self.vert))]
    ...
    def get_vector(self, connection):
        ...
        Returns
        -------
        vector: list of list of three floats representing a vector.
            list of vectors between a specific vertex and its connections.

        """
        vector = [[(connection[i][1][0] - connection[i][0][0]),(connection[i][1][1] - connection[i][0][1]),(connection[i][1][2] - connection[i][0][2])] \
    ...
    def get_vectors(self, connections):
        ...
        """
        self.vectors = [Graph_3D_edges.get_vector(self, connections[i]) \
    ...
    def plot(self):
        ...
        for entry in self.edges:
            ...
            z_list.append(entry[0][2])
            x_list.append(entry[1][0])
            y_list.append(entry[1][1])
            z_list.append(entry[1][2])
...
class Graph_3D_vertices:
    ...
    def __init__(self, vertices):
        ...
        ----------
        vertices : tuple list,
            vertices for your particular graph.
    ...
    def get_neighbors_lists(self, connections,num_neigh=None):
        ...
        """
        if num_neigh == None:
            self.neighbors_mat = Graph_3D_vertices.get_neighbors(self)
        if num_neigh != None:
            self.neighbors_mat = Graph_3D_vertices.get_neighbors_reduced(self,num_neigh)
        self.neighbors_lists = [Graph_3D_edges.get_neighbors_list(self, connections[i], self.neighbors_mat[i]) \
                                    for i in range(0,len(self.vert))]
        return self.neighbors_lists
    ...
    def get_connections(self, edges,num_neigh=None):
        ...
        if num_neigh != None:
            self.neighbors_mat = Graph_3D_vertices.get_neighbors_reduced(self,num_neigh)
        self.connections = [Graph_3D_vertices.get_connection(self, self.vert[i], edges, self.neighbors_mat[i]) \
                            for i in range(0,len(self.vert))]
        return self.connections
    ...
    def get_vectors(self, connections):
        ...
        """
        self.vectors = [Graph_3D_vertices.get_vector(self, connections[i]) \
                        for i in range(0,len(connections))]
        return self.vectors
...
def build_edge_graph_3D(edges):
    ...
    Returns
    -------
    x_graph : Graph Class 3D
        Initializes all parameters used in graph class.
    """
    x_graph = Graph_3D_edges(edges)
...
def build_vert_complete_graph_3D(vertices):
    """
    ...
    vertices : list of tuples of floats
        Describe points in 3D space.

    Returns
    -------
    x_graph : Graph Class from Verts
        Graph with all attributes of Graph Class 3D but with different initial perameters.
    """
    x_graph = Graph_3D_vertices(vertices)
...
def build_edge_complete_graph_3D(edges):
    """
...
def build_edge_reduced_graph_3D(edges,num_neigh):
    """
...
def build_vert_reduced_graph_3D(vertices,num_neigh):
    """
